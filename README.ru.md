...
- **Автоматический wakelock.** `continuum resume` теперь сам предотвращает засыпание системы
  ...
  Windows — `SetThreadExecutionState`. Wakelock освобождается автоматически после завершения
...
- **Отчёт при старте сессии.** `SessionStart`-хук проверяет `continuum-resume.log` и показывает
  сводку завершённых resume за последние 24 часа.
</details>
...
| ОС | Планировщик | Переживает ребут | Wakelock |
|---|---|---|---|
...
**Wakelock** предотвращает засыпание системы автоматически — на время ожидания до сброса
и на время выполнения задачи. Освобождается после завершения, при отмене (`kill PID`), или
при `continuum cleanup`. Не нужно вручную запускать `caffeinate`.
...
Показывает текущий процент использования прямо в статусной строке Claude Code, с цветовой
индикацией: **зелёный** (<80%), **жёлтый** (80–94%), **красный** (≥95%). Обновляется каждые
~30 секунд через фоновое обновление кеша.
...
При установке плагина скрипт копируется автоматически. При установке через однострочник
скрипт уже лежит в `~/.claude/hooks/statusline.sh`.

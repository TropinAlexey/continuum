#!/bin/sh
# Claude Code PreToolUse hook: when CONTINUUM_FRUGAL=1, block expensive tool calls.
# Blocks: Agent (subagents). Tool names are Claude's, so the list lives in the adapter.
